...
def _loadSeizures(edfFile: str, subject: str, correctedEdfFileName: str):
        ...
        while line:
            if line == 'File name: {}\n'.format(correctedEdfFileName):
                ...
                if correctedEdfFileName == 'PN01.edf':
                    ...
                    _ = summary.readline()
                    _ = summary.readline()
                    # Second annotated seizure is not in data file
                    for _ in range(2):
                        _ = summary.readline()
                ...
                # PN03 exception - Typo in seizure end time
                elif correctedEdfFileName == 'PN00-3.edf':
                    registrationStart = _parseTimeStamp(firstLine)
                    _ = summary.readline()
                    start = _parseTimeStamp(summary.readline())
                    end = summary.readline().replace('19', '18')
                    end = _parseTimeStamp(end)
                    seizures.append((_substractTimeStamps(start, registrationStart),
                                    (_substractTimeStamps(end, registrationStart))))
                # PN12 exception
...
def convertAnnotationsEdf(rootDir: str, edfFile: str, outFile: str = None) -> pd.DataFrame:
    ...
    with pyedflib.EdfReader(edfFile) as edf:
        ...
        fs_list = edf.getSampleFrequencies().tolist()
    ...
    # Channels
    channels_annot = 'all'  # TODO use lateralization info to populate channels
    ...
    for seizure in seizures:
        ...
        annotations['channels_annot'].append(channels_annot)
        annotations['filepath'].append(filepath)
        annotations['channels'].append(channel_list)
        annotations['fs'].append(fs_list)
